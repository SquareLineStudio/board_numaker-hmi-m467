...
static void _gdma_execute_drawing(lv_draw_gdma_unit_t *u);
...
void lv_draw_gdma_init(void)
{
    ...
    lv_thread_init(&draw_gdma_unit->thread, LV_THREAD_PRIO_HIGH, _gdma_render_thread_cb, 2 * 1024, draw_gdma_unit);
#endif
}
...
static int32_t _gdma_evaluate(lv_draw_unit_t *u, lv_draw_task_t *task)
{
    ...
    case LV_DRAW_TASK_TYPE_LAYER:
    {
        ...
        if (!_gdma_src_cf_supported(layer_to_draw->color_format) ||
                !_gdma_buf_aligned(layer_to_draw->draw_buf->data, layer_to_draw->draw_buf->header.stride) ||
...
static int32_t _gdma_dispatch(lv_draw_unit_t *draw_unit, lv_layer_t *layer)
{
    ...
    draw_gdma_unit->base_unit.target_layer = layer;
    draw_gdma_unit->base_unit.clip_area = &t->clip_area;
    draw_gdma_unit->task_act = t;

#if LV_USE_OS
    /* Let the render thread work. */
    if (draw_gdma_unit->inited)
        lv_thread_sync_signal(&draw_gdma_unit->sync);
#else
    _gdma_execute_drawing(draw_gdma_unit);

    draw_gdma_unit->task_act->state = LV_DRAW_TASK_STATE_READY;
...
static int32_t _gdma_delete(lv_draw_unit_t *draw_unit)
{
    ...
    draw_gdma_unit->exit_status = true;

    if (draw_gdma_unit->inited)
        lv_thread_sync_signal(&draw_gdma_unit->sync);

    lv_result_t res = lv_thread_delete(&draw_gdma_unit->thread);
...
static void _gdma_execute_drawing(lv_draw_gdma_unit_t *u)
{
    ...
    {
    case LV_DRAW_TASK_TYPE_FILL:
        lv_draw_gdma_fill(draw_unit, task->draw_dsc, &task->area);
        break;
    case LV_DRAW_TASK_TYPE_LAYER:
        lv_draw_gdma_layer((lv_draw_unit_t *)u, task->draw_dsc, &task->area);
        break;
    case LV_DRAW_TASK_TYPE_IMAGE:
        lv_draw_gdma_image((lv_draw_unit_t *)u, task->draw_dsc, &task->area);
        break;
    default:
        break;
    }
