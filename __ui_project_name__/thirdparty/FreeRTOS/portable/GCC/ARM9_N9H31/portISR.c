...
/*-----------------------------------------------------------*/
extern void systemIrqHandler(UINT32 _mIPER, UINT32 _mISNR);
/* 
 ...
 */
	...
	void  __attribute__((naked)) vPreemptiveTick( void )
	{
		...
		/* WARNING - Do not use local (stack) variables here.  Use globals
					 if you must! */
		static uint32_t iper, isnr;

		iper = inpw(REG_AIC_IPER);
		isnr = inpw(REG_AIC_ISNR);
		if(isnr != TMR1_IRQn) // TIMER1 IRQ number is 17
		{
			systemIrqHandler(iper, isnr);
		}
		else
		{
